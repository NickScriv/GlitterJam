...
void FAkSurfaceReflectorSetComponentVisualizer::PointTextAtCamera(UTextRenderComponent* textComp, float actorScaling, float maxBoundsDimension)
{
	textComp->SetWorldRotation(FQuat::Identity);

	float distanceScaling = 1.0f;
	FVector PointTo(0.0f);
	if (GCurrentLevelEditingViewportClient != nullptr)
	{
		PointTo = GCurrentLevelEditingViewportClient->GetViewLocation() - textComp->GetComponentLocation();
		distanceScaling = PointTo.Size();
	}
	if (distanceScaling > 1.0f)
	{
		// empirically derived through experimentation
		const float maxDistanceScaling = 15.0f;
		distanceScaling = FMath::Min(distanceScaling / 1000.0f, maxDistanceScaling);
	}
	textComp->SetWorldScale3D(FVector(actorScaling * distanceScaling));
	// Scale the text no larger than half the maximum bounds dimension in order to minimize text overlap (some overlap will be inevitable).
	FVector right = textComp->GetRightVector();
	right.Normalize();
	FVector textWorldSize = textComp->GetTextWorldSize();
	float textWidth = textWorldSize.ProjectOnToNormal(right).Size();
	float widthScaling = FMath::Min(1.0f, (maxBoundsDimension * 0.5f) / textWidth);
	textComp->SetWorldScale3D(FVector(actorScaling * distanceScaling * widthScaling));
...
void FAkSurfaceReflectorSetComponentVisualizer::DrawVisualization(const UActorComponent* Component, const FSceneView* View, FPrimitiveDrawInterface* PDI)
{
		...
		!SpatialAudioVolume->IsHiddenEd())
	{
			...
			++NodeIdx)
		{
			...
			if (TextComp)
			{
				float actorScaling = FMath::Max(SpatialAudioVolume->GetActorScale().GetMin(), 1.0f);
				float maxDimension = 0.0f;
				if (SurfaceReflectorSet->GetAttachParent() != nullptr)
				{
					maxDimension = SurfaceReflectorSet->GetAttachParent()->Bounds.BoxExtent.GetMax();
				}
				PointTextAtCamera(TextComp, actorScaling, maxDimension);
			}
		}
		...
		SpatialAudioVolume->FitToGeometry)
	{
		...
		if (SpatialAudioVolume->IsDragging)
		{
			...
			for (UTextRenderComponent*& TextComp : SpatialAudioVolume->PreviewTextureNameComponents)
			{
				if (TextComp != nullptr)
				{
					float actorScaling = FMath::Max(SpatialAudioVolume->GetActorScale().GetMin(), 1.0f);
					PointTextAtCamera(TextComp, actorScaling, SpatialAudioVolume->LongestEdgeLength);
				}
			}
		}
