...
namespace AkAudioBankGenerationHelper
{
	...
	void DoClearSoundData(AkSoundDataClearFlags ClearFlags)
	{
		...
		if ((ClearFlags & AkSoundDataClearFlags::AssetData) == AkSoundDataClearFlags::AssetData)
		{
			...
			{
				SlowTask.EnterProgressFrame(0.f, FText::FormatOrdered(LOCTEXT("AK_ClearAssetData", "Clearing data from asset {0}"), FText::FromName(entry.Value.AssetName)));
				if (auto* akAssetInstance = Cast<UAkAudioType>(entry.Value.GetAsset()))
				{
